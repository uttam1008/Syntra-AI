...
<style>
    ...
    [data-testid="stMetricValue"] { color: transparent; background: linear-gradient(90deg, #00C9FF 0%, #92FE9D 100%); -webkit-background-clip: text; font-weight: 900; text-shadow: 0 0 10px rgba(0, 201, 255, 0.3); }
    [data-testid="stMetricDelta"] { color: #92FE9D; }

    .telemetry-dashboard { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-top: 15px; margin-bottom: 25px; }
    .telemetry-card { background: rgba(255, 255, 255, 0.02); border: 1px solid rgba(255, 255, 255, 0.06); border-radius: 14px; padding: 20px; display: flex; flex-direction: column; justify-content: center; transition: all 0.3s ease; }
    .telemetry-card:hover { transform: translateY(-3px); border-color: rgba(217, 70, 239, 0.4); background: rgba(255, 255, 255, 0.04); box-shadow: 0 8px 25px rgba(217, 70, 239, 0.15); }
    ...
    .telemetry-value { font-size: 1.5rem; font-weight: 900; background: linear-gradient(90deg, #00C9FF 0%, #92FE9D 100%); -webkit-background-clip: text; color: transparent; line-height: 1.2; }
    .telemetry-delta { font-size: 1rem; color: #22c55e; -webkit-text-fill-color: #22c55e; margin-left: 10px; vertical-align: middle; }

    h1 { background: linear-gradient(90deg, #f87171, #f472b6, #c084fc); -webkit-background-clip: text; color: transparent !important; font-family: 'Inter', sans-serif; font-weight: 900 !important; letter-spacing: -1px; }
    h2, h3 { color: #E2E8F0 !important; font-family: 'Inter', sans-serif; }
    ...
    [data-testid="stSidebar"] .stRadio label:hover { background: linear-gradient(90deg, rgba(217, 70, 239, 0.1) 0%, rgba(255, 255, 255, 0.02) 100%); transform: translateX(6px); border-color: rgba(217, 70, 239, 0.5); border-left: 4px solid #d946ef; box-shadow: -4px 0 20px rgba(217, 70, 239, 0.2); }
    ...
    .streamlit-expanderHeader { background: rgba(255,255,255,0.02); border-radius: 8px; transition: all 0.3s; }
    .streamlit-expanderHeader:hover { background: rgba(255,255,255,0.05); color: #d946ef !important; }
</style>
...
def display_error(res):
    """Parses FastAPI errors and displays friendly text."""
    try:
        err_data = res.json()
        if res.status_code == 422:
            st.error("⚠️ Your input was too short. Please provide a few more words!")
        else:
    ...
    if st.button("Enhance Prompt", use_container_width=True):
        if not raw_prompt.strip():
            ...
            with st.spinner("📡 Orchestrating..."):
                try:
                    res = requests.post(f"{API_BASE}/enhance", json={"prompt": raw_prompt}, timeout=90)
    ...
    if st.button("Analyze Intent", use_container_width=True):
        if not raw_prompt.strip():
            ...
            with st.spinner("🧠 Analyzing Cognition..."):
                try:
                    res = requests.post(f"{API_BASE}/intent", json={"prompt": raw_prompt}, timeout=90)
                    if res.status_code == 200:
                        data = res.json()
        ...
        if not raw_prompt.strip():
            ...
            with st.spinner("🗜️ Compressing semantics..."):
                try:
                    res = requests.post(
                        f"{API_BASE}/compress",
                        json={"input_text": raw_prompt, "preserve_code": True, "mode": mode},
                        timeout=90
                    )
                    if res.status_code == 200:
                        data     = res.json()
...
# ─── Intelligent Router ────────────────────────────────────────────────────────
    ...
    **Stop guessing which AI agent or tool you need.**
    Simply describe what you want to build, and the Orchestrator will instantly analyze your complex request,
    assemble a specialized team of AI experts, and map out the exact execution pipeline.
    """)
                    ...
                    if res.status_code == 200:
                        ...
                        if pipeline:
                            pipeline_html = " <br>⬇<br> ".join([
                                f"<div style='background-color: #1e293b; padding: 10px; border-radius: 5px; text-align: center; border: 1px solid #334155;'><b>{step}</b></div>"
